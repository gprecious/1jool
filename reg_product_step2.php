...
<?php
	...
	try {
		...
		$pd_opt = $_POST['pd_opt'];
		$pd_opt_price = $_POST['pd_opt_price'];
		$optCount = 0;	//option 에 몇개가 들어왔는지 판단
		foreach ($pd_opt as $value) {
			if($value != "") {
				$optCount++;
			}
		}
		...
		$getLastOptNoQuery = "SELECT `no` FROM `yh_product_option` ORDER BY `no` DESC LIMIT 1";
	...
	try {
		...
		VALUES ('$newPdNo', '$pd_name', '$pd_shortDesc', 'temp_code', '$pd_price', NULL, '$nowDate', NULL, '1')";
		...
		for ($i=0; $i < $optCount; $i++) { 
			$insertOptionQuery = "INSERT INTO  `bigtrue`.`yh_product_option` (
				`no` ,
				`pd_no` ,
				`pdo_title` ,
				`pdo_additionalPrice`
			...
			VALUES (
				'" . ($newOptNo + $i) . "', '$newPdNo', '$pd_opt[$i]', '$pd_opt_price[$i]'
			)";
